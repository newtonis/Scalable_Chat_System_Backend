...
async def test_subscribe_to_conversation(client, valid_token, user_id):
    ...
    async with websockets.connect(
        ...
        res = await client.post(
            f"{constants.COORDINATOR_URL}/api/subscribe_dm",
...
async def test_send_direct_message(client, valid_token, user_id):
    ...
    async with websockets.connect(
        ...
        data_package = {
            "command": "new_message",
            "message": {"type": "dm", "user_id": user_id, "value": "test message"},
        }
        ...
        assert (
            rta_json["message"]["value"] == "test message"
            and int(rta_json["message"]["user_id"]) == user_id
            and rta_json["message"]["type"] == "dm"
        )
...
async def test_send_peer_message(client, func_register_and_login_new_user):
    ...
    async def task_message(
        ...
        async with websockets.connect(
            ...
            assert (
                my_message["value"] == f"Hello from user {from_id}"
                and my_message["user_id"] == from_id
                and my_message["type"] == "dm"
            )
            assert (
                peer_message["value"] == f"Hello from user {to_id}"
                and peer_message["user_id"] == to_id
                and peer_message["type"] == "dm"
            )
